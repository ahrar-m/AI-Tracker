...
A lightweight, visually stunning HTML5 dashboard to track weekly and monthly usage limits for any AI system. It calculates real-time consumption progress, highlights whether you are leading or lagging the ideal linear utilization target, and displays a live countdown to your next quota resets.
...
- **No Dependencies**: A simple three-file app — `index.html`, `style.css`, `app.js` — with no build step or installations. Data is stored privately in browser LocalStorage. (Note: the Google Fonts are fetched from the network when online; the app falls back to system fonts otherwise.)
...
1. Open `index.html` in any modern web browser, or visit the GitHub Pages site.
